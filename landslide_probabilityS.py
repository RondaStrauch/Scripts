...
"""
landslide_probabilityS.py:

Landlab component that simulates relative wetness and probability of failure.
VERSION landslide_probabilityS.py eliminates the minimum soil depth of 0.005
and adds 'temporary lines' (search) for saving relative wetness distribution.
...
a distributio of relative wetness (Search NEW SECTION in code).

.. codauthor:: R.Strauch, E.Istanbulluoglu, & S.Nudurupati
University of Washington
Created on Thu Aug 20, 2015
Last edit May 17, 2017
"""
...
# Run Component
    ...
    def calculate_factor_of_safety(self, i):
        ...
        hs_min = self.hs_mode-0.3*self.hs_mode  # for SSURGO-SD only
        ...
        for val in self.Rel_wetness:               # find how many RW values >= 1
            if val >= 1.0:
                countr = countr + 1
        self.RW_G1 = float(countr)     # number with RW values (>=1)
    ...
    def _interpolate_HSD_dict(self):
        ...
        for vkey in HSD_dict.keys():
            ...
            Re_temp = HSD_dict[vkey]	# array of annual Re for 1 HSD grid
